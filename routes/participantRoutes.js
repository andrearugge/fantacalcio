...
try {
  ...
  participantController = {
    createParticipant: (req, res) =>
      res.status(500).json({ error: "Participant controller not available" }),
    getParticipantByLink: (req, res) =>
      res.status(500).json({ error: "Participant controller not available" }),
  };
}

router.post("/", participantController.createParticipant);
router.get("/link/:uniqueLink", participantController.getParticipantByLink);
